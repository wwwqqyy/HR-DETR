...
class ConvAdapter(nn.Layer):
    ...
    def __init__(self, inplanes, outplanes, width,
        ...
        # depth-wise conv
        self.conv1 = nn.Conv2D(inplanes, width, kernel_size=kernel_size, stride=stride, groups=groups, padding=padding,
        ...
        x = 1.0 * paddle.ones((1, outplanes, 1, 1))
        self.se = paddle.create_parameter(shape=x.shape, dtype=x.dtype,
                                          default_initializer=paddle.nn.initializer.Assign(x))
        self.se.stop_gradient = False
...
class BottleNeck(nn.Layer):
    ...
    def __init__(self,
        ...
        self.adapter = ConvAdapter(width, width,
                                   kernel_size=3,
                                   padding=1,
                                   width=int(width // 8),
                                   stride=stride,
...
class Blocks(nn.Layer):
    ...
    def forward(self, inputs, flag):
        ...
        for block in self.blocks:
            block_out = block(block_out, flag)
        return block_out, flag
