...
class AddBranch(nn.Layer):
    def __init__(self, in_chn, out_chn):
        super(AddBranch, self).__init__()
        self.conv1 = nn.Sequential(
            nn.Conv2D(in_chn, in_chn, kernel_size=3, padding=1),
            nn.ReLU(),
        )
        ...
        self.conv_out = nn.Sequential(
            nn.Conv2D(in_chn, out_chn, kernel_size=1, padding=0),
            nn.BatchNorm2D(out_chn, momentum=0.0003),
            nn.ReLU(),
        )
...
class DiffBranch(nn.Layer):
    def __init__(self, in_chn, out_chn):
        super(DiffBranch, self).__init__()
        self.conv1 = nn.Sequential(
            nn.Conv2D(in_chn, in_chn, kernel_size=3, padding=1),
            nn.ReLU(),
        )
...
class Retention(nn.Layer):
    def __init__(self, dim_in, dim_out, reduction=True):
        super(Retention, self).__init__()
        if reduction:
            self.reduction = nn.Sequential(
                nn.Conv2D(dim_in, dim_in // 2, kernel_size=1, padding=0),
                nn.BatchNorm2D(dim_in // 2, momentum=0.0003),
                nn.ReLU(),
            )
            dim_in = dim_in // 2
        else:
            self.reduction = None
        self.cat1 = AddBranch(dim_in, dim_out)
        self.cat2 = DiffBranch(dim_in, dim_out)
        self.conv1 = nn.Sequential(
            nn.Conv2D(dim_out, dim_out, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2D(dim_out, momentum=0.0003),
            nn.ReLU(),
        )
