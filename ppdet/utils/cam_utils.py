...
def draw_feature_map(
    features: Union[paddle.Tensor, List[paddle.Tensor]],
    img_path: Union[str, np.ndarray],
    stage: int,
    save_dir: str = './work_dirs/feature_map/',
    heatmap_intensity: float = 0.4,
    colormap: int = cv2.COLORMAP_JET,
    show: bool = False
) -> None:
    """
    将特征图可视化为热力图，并叠加到原始图像上保存（Linux 版本）。

    Args:
        features (Union[paddle.Tensor, List[paddle.Tensor]]): 特征图，可以是单层或多层。
        img_path (Union[str, np.ndarray]): 测试图像的文件路径或已经加载的图像数据。
        stage (int): 当前处理的阶段标识（用于生成保存文件名）。
        save_dir (str): 保存生成图像的文件夹路径。
        heatmap_intensity (float): 热力图叠加的透明度（0-1之间）。
        colormap (int): OpenCV 颜色映射类型，默认为 JET。
        show (bool): 是否显示叠加后的图像。
    """
    # 检查输入
    if not isinstance(features, (paddle.Tensor, list)):
        raise ValueError("`features` 必须是 paddle.Tensor 或列表类型。")
    if isinstance(features, paddle.Tensor):
        features = [features]  # 统一转为列表处理
    ...
    if isinstance(img_path, str):
        img = cv2.imread(img_path)  # 从文件路径加载图像
        if img is None:
            raise FileNotFoundError(f"无法加载图像文件：{img_path}")
    else:
        img = img_path  # 直接使用传入的图像数据
    img = img_path.squeeze(0).transpose((1, 2, 0))
    # 图像预处理
    if img.ndim == 3 and img.shape[0] == 3:  # 如果图像是 [C, H, W] 格式
        img = img.transpose((1, 2, 0))  # 转换为 [H, W, C]
    img = img.astype(np.uint8)  # 确保图像为 uint8 类型

    # 遍历特征图并生成热力图
    for i, feature_map in enumerate(features):
        heatmap = featuremap_to_heatmap(feature_map)  # 生成热力图
        heatmap_resized = cv2.resize(heatmap, (img.shape[1], img.shape[0]))  # 调整热力图大小

        # 将热力图转换为 uint8 类型
        heatmap0 = (255 * heatmap_resized).astype(np.uint8)

        # 应用颜色映射
        heatmap_colored = cv2.applyColorMap(heatmap0, colormap)
        # import pdb
        # pdb.set_trace()
        # 将热力图叠加到原始图像上
        superimposed_img = heatmap_colored * 0.5 + img.cpu().numpy()

        # 显示结果（可选）
        if show:
            cv2.imshow(f"Stage {stage} - Feature {i}", superimposed_img)
            cv2.waitKey(0)
            cv2.destroyAllWindows()

        # 保存结果
        save_path = os.path.join(save_dir, f"stage{stage}_feature{i}.png")
        cv2.imwrite(save_path, superimposed_img)
        print(f"Saved: {save_path}")


def featuremap_to_heatmap(feature_map: paddle.Tensor) -> np.ndarray:
    """
    将特征图转换为热力图。

    Args:
        feature_map (paddle.Tensor): 输入的特征图，格式为 [B, C, H, W] 或 [C, H, W]。

    Returns:
        np.ndarray: 热力图，格式为 [H, W]，值范围为 [0, 1]。
    """
    if not isinstance(feature_map, paddle.Tensor):
        raise ValueError("`feature_map` 必须是 paddle.Tensor 类型。")
    feature_map = feature_map.detach()
    # 去除批次维度（如果存在）
    if feature_map.ndim == 4:
        feature_map = feature_map.squeeze(0)  # [B, C, H, W] -> [C, H, W]

    # 计算热力图
    heatmap = paddle.sum(feature_map, axis=0).numpy()  # 按通道求和 [C, H, W] -> [H, W]
    # import pdb
    # pdb.set_trace()
    heatmap = np.maximum(heatmap, 0)  # 去除负值
